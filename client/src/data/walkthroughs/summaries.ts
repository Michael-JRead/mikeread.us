...
export interface WalkthroughSummary {
  ...
  retired: string;
  tags: string[];
  /** Single-sentence attack chain — used as the postbuild meta description
   *  for the per-walkthrough page. Kept alongside `bullets` so search-engine
   *  descriptions stay well-formed. */
  summary: string;
  /** Scannable bullet takeaways — rendered on the walkthrough card. */
  bullets: string[];
}
...
export const WALKTHROUGH_SUMMARIES: WalkthroughSummary[] = [
  {
    ...
    bullets: [
      "Pre-auth CraftCMS RCE (CVE-2025-32432) → www-data shell",
      "`.env` → bcrypt → SSH lateral as adam",
      "telnetd USER-env auth bypass (CVE-2026-24061) → root",
    ],
  },
  {
    slug: "nexus",
    ...
    bullets: [
      "Gitea history leak surfaces dev creds",
      "Krayin CRM CVE-2026-38526 TinyMCE upload → RCE",
      "`.env` password reuse + root-owned Gitea template sync writes `authorized_keys` via path traversal",
    ],
  },
  {
    slug: "odyssey",
    ...
    bullets: [
      "NoSQL leak + WebAuthn userHandle confusion + prototype pollution",
      "LaTeX file read → jsonpath-plus RCE",
      "MSSQL coercion → GodPotato → BadSuccessor dMSA",
      "YAML deserialization on the DC → domain compromise",
    ],
  },
  {
    slug: "sorcery",
    ...
    bullets: [
      "Unparameterized Cypher injection unravels a microservice mesh",
      "Chain: XSS → SSRF → hand-built Kafka frame → cracked CA → FreeIPA",
      "Full root",
    ],
  },
  {
    ...
    bullets: [
      "Uptime Kuma leak + HMAC-signed webhook SQLi",
      "restic backup abuse → Docker escape",
      "Clock-seeded password generator cracks root",
    ],
  },
  {
    slug: "thefrizz",
    ...
    bullets: [
      "CVE-2023-45878 file write in Gibbon-LMS → web shell",
      "Cracked salted SHA256 for Kerberos SSH",
      "Recycle Bin WAPT backup + GPO abuse → SYSTEM",
    ],
  },
  {
    slug: "zero",
    ...
    bullets: [
      "Self-service SFTP creds → foothold",
      "`.htaccess` arbitrary file read",
      "Root Apache config-check cron → full root",
    ],
  },
  {
    slug: "escapetwo",
    ...
    bullets: [
      "Corrupt-XLSX cred recovery → MSSQL xp_cmdshell",
      "Reused service password for lateral",
      "WriteOwner ACL + ADCS ESC4→ESC1 certificate abuse → Domain Admin",
    ],
  },
  {
    slug: "ghost",
    ...
    bullets: [
      "LDAP wildcard bypass + container foothold",
      "gMSA capture → Golden SAML",
      "Cross-realm golden ticket → Enterprise Admin",
    ],
  },
  {
    slug: "certified",
    ...
    bullets: [
      "Assumed-breach start on AD",
      "WriteOwner over a group → Shadow Credentials on service accounts",
      "ADCS ESC9 UPN-swap forges Administrator certificate → domain compromise",
    ],
  },
  {
    slug: "cicada",
    ...
    bullets: [
      "Guest SMB + RID-cycling → user list",
      "Password-in-LDAP-description leak",
      "Backup Operator's SeBackupPrivilege → Domain Admin Pass-the-Hash",
    ],
  },
  {
    slug: "magicgardens",
    ...
    bullets: [
      "SSRF-forced subscription + QR-code XSS reach Django admin",
      "harvest buffer overflow → leaked registry image",
      "Forged pickle session + CAP_SYS_MODULE → host root",
    ],
  },
  {
    slug: "sea",
    ...
    bullets: [
      "Phish WonderCMS admin bot with CVE-2023-41425 XSS-to-RCE",
      "Crack bcrypt hash for SSH password reuse",
      "Command-inject loopback log analyzer running as root",
    ],
  },
  {
    slug: "mist",
    ...
    bullets: [
      "Pluck CMS file-read → RCE",
      "ADCS abuse: PKINIT + NTLM relay + shadow creds",
      "Double ESC13 → Domain Admin",
    ],
  },
  {
    slug: "skyfall",
    ...
    bullets: [
      "nginx/Flask parser differential unlocks a metrics page leaking MinIO",
      "MinIO CVE-2023-28432 discloses root keys → versioned backup yields Vault token",
      "Chatty sudo unseal binary captured over go-fuse gives up root Vault token → SSH-OTP root",
    ],
  },
  {
    slug: "ouija",
    ...
    bullets: [
      "HAProxy request smuggling reaches a filtered dev vhost",
      "SHA-256 length-extension forge unlocks API file-read",
      "Integer overflow in a bespoke PHP C extension → root arbitrary write",
    ],
  },
  {
    slug: "twomillion",
    ...
    bullets: [
      "Reverse an obfuscated invite flow",
      "Abuse broken-access-control API to self-promote to admin",
      "Blind-inject VPN generator → shell; reuse `.env` creds over SSH",
      "OverlayFS (CVE-2023-0386) → root",
    ],
  },
  {
    slug: "flight",
    ...
    bullets: [
      "LFI on flight-school vhost coerces svc_apache's Net-NTLMv2 hash to Responder",
      "Cracking, spraying, desktop.ini forced-auth across S.Moon and c.bum → webshell",
      "JuicyPotatoNG on IIS apppool → SYSTEM",
    ],
  },
  {
    slug: "response",
    ...
    bullets: [
      "Weaponise HMAC signing oracle into SSRF proxy",
      "Bypass LDAP auth with a rogue server + forge cross-protocol FTP requests",
      "Backdoored ssl-cert NSE script + Meterpreter decrypt rebuilds root's SSH key",
    ],
  },
  {
    slug: "scanned",
    ...
    bullets: [
      "Downloadable chroot/ptrace sandbox leaks two bugs: dumpable-before-fork + leaked outer-dir fd",
      "Arbitrary file read via /proc/1/fd/3 → cracked Django creds for SSH",
      "Trojaned libpam_misc.so.0 loaded into setuid su → root",
    ],
  },
  {
    slug: "timelapse",
    ...
    bullets: [
      "Anonymous SMB yields a zip hiding a WinRM client cert",
      "PowerShell history leaks a service account",
      "LAPS read rights expose the DC's local Administrator password",
    ],
  },
  {
    slug: "cap",
    ...
    bullets: [
      "IDOR on Flask/Gunicorn security dashboard leaks a PCAP with cleartext FTP creds for nathan",
      "FTP creds reused over SSH",
      "cap_setuid on /usr/bin/python3.8 → root",
    ],
  },
  {
    slug: "sauna",
    ...
    bullets: [
      "Harvest AD usernames from bank site → AS-REP roast fsmith",
      "Loot AutoLogon creds for svc_loanmgr",
  ...
  },
  {
    ...
    bullets: [
      "Null-session enumeration finds svc-alfresco",
      "AS-REP roasting cracks it",
      "Exchange WriteDACL ACL grants DCSync → dump Administrator hash → pass-the-hash → root",
    ],
  },
  {
    slug: "bastion",
    ...
    bullets: [
      "Anonymous SMB exposes a backup VHD",
      "Mount with guestmount, dump SAM/SYSTEM, crack L4mpje's NTLM hash",
      "Decrypt an mRemoteNG saved credential → SSH as Administrator",
    ],
  },
  {
    slug: "rope",
    ...
    bullets: [
      "LFI-assisted format-string GOT overwrite lands a shell",
      "World-writable liblog.so hijack pivots users",
      "Canary-brute ROP chain against a root service on 1337 → root",
    ],
  },
  {
    slug: "active",
    ...
    bullets: [
      "Anonymous SMB reads a GPP cpassword for SVC_TGS",
      "Kerberoast the Administrator SPN",
      "Crack the TGS hash → Domain Admin + PsExec SYSTEM",
    ],
  },
  {
    slug: "nibbles",
    ...
    bullets: [
      "Hidden Nibbleblog 4.0.3 install + guessable admin:nibbles creds",
      "My Image plugin file-upload (CVE-2015-6967) → shell as nibbler",
      "sudo NOPASSWD rule on writable monitor.sh → root",
    ],
  },
  {
    slug: "legacy",
    ...
    bullets: [
      "Unpatched Windows XP SP3 host exposes SMBv1",
      "Vulnerable to both MS17-010 (EternalBlue) and MS08-067 (NetAPI)",
      "One unauthenticated exploit → direct SYSTEM shell",
    ],
  },
  {
    slug: "lame",
    ...
    bullets: [
      "Unauthenticated Samba 3.0.20 username-map-script injection (CVE-2007-2447) → direct root",
      "Alternate distccd RCE foothold",
      "Escalated via SUID nmap's interactive mode",
    ],
  },
];
...
// Card shape consumed by the /hackthebox walkthrough grid.
